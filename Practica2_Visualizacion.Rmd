...
g <- grafico1+geom_point()

grafico2<- ggplot(ds, aes(x = MinTemp, y = MaxTemp, colour=RainTomorrow))+geom_point()

grafico3 <- grafico2+ggtitle("Grafico de dispersion MinTemp frente MaxTemp")
grafico3
...
g1 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, colour=RainToday))+geom_point()
g1

g2<-ggplot(ds, aes(x = Temp9am, y = Pressure9am, shape=RainToday))+geom_point()
g2

g3 <- g1+ggtitle("Grafico de dispresion Pressure9am vs Temp9am")
g3


#La informacion del grafico se observa mejor con la representacion en color ya que en forma se mezclan y no se ve claro.
...
str(iris) #Iris tiene 5 variables
...
g1 <- ggplot(iris.tidy, aes(x = Species, y = Value))+geom_point() + facet_grid(Measure~.)

g2 <- ggplot(iris.tidy, aes(x = Species, y = Value))+geom_point() + facet_grid(.~Measure)

#Los valores se pueden comparar mejor con la disposicion en filas ya que se ven mucho mas claros los datos de las variables.

iris.grafico3 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour=Part))+geom_jitter()
iris.grafico3

#Se observa que utilizando geom_jittel() los puntos salen dispersados por todo el grafico, es decir, los datos no están exactamente en las coordenadas mientras que usando geom_point los puntos salen unicamente en la Specie (coordenadas) a la que ue pertenece.
...
str(DatosEjer4) #Este dataset tiene 3 variables que son "fecha", "fabricante" y "ventas"

g1 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, colour=Fabricante))+geom_point()

#En este grafico podemos observar que Microsoft tiene mas ventas que Apple al largo de los años, pero para poder apreciar más concretamente en que momentos vende más uno que otro es mejor hacer un grafico que una los puntos.

g2 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, colour=Fabricante))+geom_line()+geom_point()

g3 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, group=Fabricante))+geom_line(aes(linetype=Fabricante))+geom_point()

g4 <- g3+ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
g4
...
str(Experimento) #Este dataset tiene 4 variables que son "complemento", "dosis", "altura" y "error_altura"

p1 <- ggplot(Experimento, aes(x = Dosis, y = Altura, group=Complemento))+geom_line(aes(linetype=Complemento, color=Complemento))+geom_point(aes(shape=Complemento,color=Complemento))
p1

p <- ggplot(Experimento, aes(x = Dosis, y = Altura, group=Complemento, shape=Complemento, linetype=Complemento))+geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))+geom_line()+geom_point()+labs(title="Gráfico de la variable Dosis frente a Altura",x="Dosis", y = "Altura")+
theme_classic()
p

p2 <- ggplot(Experimento, aes(x=Dosis, y=Altura, group = Complemento, color=Complemento))+
geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1,
position=position_dodge(0.05)) +
geom_line(aes(linetype=Complemento)) +
geom_point(aes(shape=Complemento))+
labs(title="Gráfico de la variable Dosis frente a Altura",x="Dosis", y = "Altura") +
theme_classic()
p2+theme_classic() + scale_color_brewer(palette="Reds")
...
s1 <- ggplot(data=ds, aes(x=Year, y = MinTemp))+geom_boxplot()
s1

s2 <- ggplot(data=ds, aes(x=Year, y = MinTemp, fill=Year))+geom_violin()
s2

s3 <- s2 + labs(title = "Distribución de temperaturas máximas por año, x = Año, y = Temperatura (ºC)")
s3

s4 <- s3 + theme(axis.text.x = element_text(angle = 45), legend.position = "none")
s4
...
g1 <- ggplot(data = dataset, aes(x = HEIGHT)) + geom_histogram()
g1

g2 <- g1 + labs(title = "Histograma de la variable Altura, x = Altura (cm), y = Frecuencia")
g2

g3 <- ggplot(data = dataset, aes(x = HEIGHT)) + geom_histogram(breaks = seq(155,195, by = 2), bins = 20) + labs(title = "Historiograma de la variable Altura", x ="Altura", y ="Frecuencia")
g3

g4 <- ggplot(data = dataset, aes(x = HEIGHT)) + geom_histogram(breaks = seq(155,195, by = 2), bins = 20, fill = "blue", col = "red", alpha = 0.5) + labs(title = "Historiograma de la variable Altura", x ="Altura", y ="Frecuencia")
g4

g5 <- ggplot(data = dataset, aes(x = HEIGHT)) + geom_histogram((aes(y = stat(density))),breaks = seq(155,195, by = 2), bins = 20, fill = "blue", col = "red", alpha = 0.5) + geom_density(col = "green")+labs(title = "Historiograma de la variable Altura", x ="Altura", y ="Frecuencia")
g5

g6 <- ggplot(data = dataset, aes(x = HEIGHT)) + geom_histogram((aes(y = stat(density))),breaks = seq(155,195, by = 2), bins = 20, fill = "blue", col = "red", alpha = 0.5) + geom_density(col = "green")+facet_grid(.~SMOKE)+labs(title = "Historiograma de la variable SMOKE", x ="Smoke", y ="Frecuencia")
g6
